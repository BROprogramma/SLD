...
<!--XSLT IMBRO SLD  Complete Report versie 1.0 (29-7-2025) - SIKB0101 versie 14.9.0-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsi="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdt="http://www.w3.org/2005/xpath-datatypes" xmlns:imsikb0101="http://www.sikb.nl/imsikb0101" xmlns:immetingen="http://www.sikb.nl/immetingen" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gsr="http://www.isotc211.org/2005/gsr" xmlns:gss="http://www.isotc211.org/2005/gss" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:om="http://www.opengis.net/om/2.0" xmlns:sam="http://www.opengis.net/sampling/2.0" xmlns:sams="http://www.opengis.net/samplingSpatial/2.0" xmlns:spec="http://www.opengis.net/samplingSpecimen/2.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sikb="http://xslcontrole.sikb" xsi:schemaLocation="http://www.sikb.nl/imsikb0101 imsikb0101_v14.8.0.xsd">
	...
	<xsl:template match="/">
		<ArrayOfLogRecord>	
            ...
            <xsl:if test="count(//imsikb0101:Remediation) != 1">
                <!-- Check existence Project -->
                <xsl:variable name="message" select="'In het xml-bestand moet minimaal en maximaal 1 Remediation zijn opgenomen.'"/>
	...
	<xsl:template match="imsikb0101:metaData">
		...
		<xsl:copy-of select="sikb:checkDateBeforeDate(., '', 'date', 'current', 'ERROR')"/>
	</xsl:template>
	...
	<xsl:template match="imsikb0101:Remediation">
		...
		<xsl:copy-of select="sikb:checkFilled(., $prGUID, 'endTime', 'ERROR')"/>
		...
		<!-- mogen niet aanwezig zijn, dus 0 characters -->
		<xsl:copy-of select="sikb:checkLength(., $prGUID, 'realizedVariationTopsoil', 0, 'ERROR')"/>
		<xsl:copy-of select="sikb:checkLength(., $prGUID, 'realizedVariationSubsoil', 0, 'ERROR')"/>
	
							
		<!-- Voor WBB mag er geen Approach aanwezig zijn, deze wordt genegeerd en leeg gelaten. Bij OW verplicht -->	
		<xsl:copy-of select="sikb:checkExistence(., $prGUID,'approachType','ERROR')" />
		<xsl:copy-of select="sikb:checkFilled(., $prGUID, 'approachType', 'ERROR')"/>
		<xsl:copy-of select="sikb:checkLookupId(., $prGUID, 'approachType', 'AanpakVerontreiniging', 'ERROR')"/>
	
        <xsl:if test="count(imsikb0101:project) != 1">
            <!-- Check existence Project -->
            <xsl:variable name="message" select="'In het xml-bestand moet exact 1 Project gekoppeld zijn aan de Remediation, van het projectType 11 of 44.'"/>
            <xsl:copy-of select="sikb:createRecord('ERROR', 'xml-bestand', $message)"/>
        </xsl:if>      
				
		<xsl:for-each select="imsikb0101:project">
            <xsl:variable name="projectId" select="replace(./@xlink:href, '#','')"/>                        
            <xsl:variable name="project" select="//imsikb0101:Project[@gml:id = $projectId]"/>                 
...
<xsl:variable name="valueDate">
	...
	<xsl:function name="sikb:checkDependancyDecisionProject">
        ...
        <xsl:variable name="decisionTypeValue" select="key('imsikbKey', concat('besluit', '|', $decisionTypeValue), $imsikbLookup)/waarde"/>
        <xsl:variable name="projectTypeValue" select="key('imsikbKey', concat('onderzoekType', '|', $projectType), $imsikbLookup)/waarde"/>
